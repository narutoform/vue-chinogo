            ...
            <div class="fix-bottom-inner">
              <div class="cart-bar-operation">
                <div>
                  ...
                  <div class="delete-choose-goods" @click="deleteCartAll()">删除全部购物车
                  </div>
                </div>
              </div>
              <div class="shipping">
...
<script>
  import { cartEdit, editCheckAll, delCart, deleteCartAll } from '/api/goods'
  ...
  export default {
    ...
    methods: {
      ...
      editCheckAll () {
        ...
        req.append('checkAll', checkAll)
        editCheckAll(req).then(res => {
      ...
      _cartEdit (productId, productNum, checked) {
        let req = new URLSearchParams()
        req.append('productId', productId)
        req.append('productNum', productNum)
        req.append('checked', checked)
        cartEdit(req)
          .then(res => {
            if (res.status === '0') {
              this.EDIT_CART(
      ...
      cartdel (productId) {
        delCart(productId)
          .then(res => {
            ...
            this.$message.success('删除当前购物车商品成功')
          })
          .catch(res => {
            this.$message.error('删除当前购物车商品失败')
          })
      },
      // 删除全部购物车
      deleteCartAll () {
        deleteCartAll()
          .then(res => {
            this.REDUCE_CART()
            this.$message.success('删除全部购物车成功')
          })
          .catch(res => {
            this.$message.error('删除全部购物车失败')
          })
      },
      checkout () {
