    ...
    <div>
      <header class="w">
        <div class="w-box">
          <div class="nav-logo">
            <h1>
              <router-link to="/" title="chinogo官网">chinogo</router-link>
            </h1>
          </div>
          <div class="right-box">
          ...
          <div class="nav-sub-wrapper" :class="{fixed:st}">
            <div class="w">
              <ul class="nav-list">
                ...
                <li>
                  <el-input placeholder="请输入内容进行搜索" style="width: 600px;" v-model="q">
  ...
  export default {
    ...
    methods: {
      ...mapMutations(['ADD_CART', 'INIT_BUYCART', 'ADD_ANIMATION', 'SHOW_CART',
        'RECORD_USERINFO', 'EDIT_CART', 'SEARCH_ITEM_PARAM',
        'SEARCH_ITEM', 'CLEAR_SEARCH_ITEM']),
      // 购物车显示
      cartShowState (state) {
      ...
      searchClick () {
        ...
        // 清空查询结果
        this.CLEAR_SEARCH_ITEM()
        // 添加查询结果集合
        this.SEARCH_ITEM_PARAM({q: this.q, page: 1})
        this.SEARCH_ITEM()
        this.$router.push({
          path: '/goods',
          query: {
...
<style lang="scss" rel="stylesheet/scss" scoped type="text/scss">
  ...
  .w-box {
    ...
    h1 {
      ...
      align-items: center;
      > a {
        background: url(/static/images/main.png) no-repeat 50%;
